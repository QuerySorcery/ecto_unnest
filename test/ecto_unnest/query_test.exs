defmodule EctoUnnest.QueryTest do
  ...
  describe "validation" do
    ...
    test "require_all_fields: missing schema field -> error listing it" do
      assert_raise ArgumentError, ~r/schema fields \[:payload, :score, :tags\] are not provided/, fn ->
    ...
    test "require_all_fields: every field covered (nil placeholder for an unset one) -> ok" do
      {sql, _} =
        EctoUnnest.to_sql(
          Event,
          %{user_id: [1], type: ["a"], score: [1.0], payload: [nil]},
          placeholders: %{inserted_at: ~U[2026-06-17 10:00:00Z], tags: nil},
          require_all_fields: true
        )
    ...
    test "require_all_fields: autogenerated primary key is exempt" do
      # :id is not provided, but it is the autogenerate primary key -> no error.
      {_sql, _} =
        EctoUnnest.to_sql(
          Event,
          %{user_id: [1], type: ["a"], score: [1.0], payload: [nil]},
    ...
    test "require_all_fields: rejects a binary source (no field list)" do
      assert_raise ArgumentError, ~r/needs a schema/, fn ->
        EctoUnnest.to_sql("events", %{user_id: [1]},
          types: %{user_id: :bigint},
          require_all_fields: true
        )
      end
    end
